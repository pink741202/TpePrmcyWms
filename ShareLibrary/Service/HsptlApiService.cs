        ...
        public async Task<employee?> getEmpInfo(string emp_no)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    qwFunc.log($"Api getEmpInfo({emp_no}) 回傳:{response}");
                    ApiEmployeeRoot Root = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiEmployeeRoot>(response);
                    if(Root.msg != "") { qwFunc.log($"Api getEmpInfo({emp_no}) 回傳 Msg: {Root.msg}"); }
                    ...
                    qwFunc.log($"Api getEmpInfo({emp_no}) 成功");
                    return result;
                }
            ...
            {
                qwFunc.log($"Api getEmpInfo({emp_no}) 失敗:{ex}");
                return null;
            }
        }

        public async Task<ResponObject<employee?>> EmpRequest(string emp_no)
        {
            if (string.IsNullOrEmpty(emp_no)) { return new ResponObject<employee?>("1", null, "無提供查詢資料"); }
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    ApiEmployeeRoot Root = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiEmployeeRoot>(response);
                    if (Root.msg != "") { qwFunc.log($"Api getEmpInfo({emp_no}) 回傳 Msg: {Root.msg}"); }
                    if (Root.status.ToLower() != "success" || Root.rows.Count == 0 || Root.rows.First().emp_no != emp_no) { return new ResponObject<employee?>("1", null, Root.msg); }
                    var row = Root.rows.First();
                    ...
                    qwFunc.log($"Api getEmpInfo({emp_no}) 成功");
                    return new ResponObject<employee?>("0", result, ""); 
                }
            ...
            {
                qwFunc.log($"Api getEmpInfo({emp_no}) 失敗:{ex}");
                return new ResponObject<employee?>("Err", null, ex.Message);
            }
        }
        public async Task<DrugInfo?> getDrugInfo(string odr_code) 
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    qwFunc.log($"Api getDrugInfo({odr_code}) 回傳:{response}");
                    ApiMedicineRoot Root = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiMedicineRoot>(response);
                    if (Root.msg != "") { qwFunc.log($"Api getEmpInfo({odr_code}) 回傳 Msg: {Root.msg}"); }
            ...
            catch (Exception ex) { qwFunc.log($"Api getDrugInfo({odr_code}) 失敗:{ex}");  return null; }
        }

        public async Task<ResponObject<DrugInfo?>> DrugRequest(string odr_code)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    ApiMedicineRoot Root = Newtonsoft.Json.JsonConvert.DeserializeObject<ApiMedicineRoot>(response);
                    if (Root.msg != "") { qwFunc.log($"Api getEmpInfo({odr_code}) 回傳 Msg: {Root.msg}"); }
                    if (Root.status.ToLower() != "success" || Root.rows.Count == 0 || Root.rows.First().odr_code != odr_code) { return new ResponObject<DrugInfo?>("1", null, Root.msg); }
                    var row = Root.rows.First();

                    DrugInfo? result = new DrugInfo
                    {
                        DrugCode = row.odr_code,
                        DrugName = row.textinfo_01,
                        Info_01 = row.textinfo_01,
                    ...
                    return new ResponObject<DrugInfo?>("1", result, "");
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getDrugInfo({odr_code}) 失敗:{ex}"); return new ResponObject<DrugInfo?>("Err", null, ex.Message); }
        }

        public async Task<bool?> getOutStorageAsync(Qry_OutStorage qry)
        {
            bool? result;
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    else { qwFunc.log($"Api getOutStorage({qry.OdrCode}...) 回傳 Msg:{resp.Message}"); }
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getOutStorage({qry.OdrCode}...) 失敗:{ex}"); return null; }
            return null;
        }

        public bool? getOutStorage(Qry_OutStorage qry)
        {
            bool? result;
            try
            {
        ...
        public async Task<List<Data_ReturnByQRCode>> getOPDReturnStorageByQRCode(Qry_ReturnByQRCode qry)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (resp.Succ) { return resp.Data; }
                    else { qwFunc.log($"Api getOPDReturnStorageByQRCode({qry.PatNo}...) 回傳 Msg:{resp.Message}"); }
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getOPDReturnStorageByQRCode({qry.PatNo}...) 失敗:{ex}"); return null; }
            return null;
        }
        ...
        public async Task<List<Data_ReturnByHand>> getOPDReturnStorageByHand(Qry_ReturnByHand qry)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (resp.Succ) { return resp.Data; }
                    else { qwFunc.log($"Api getOPDReturnStorageByHand({qry.PatNo}...) 回傳 Msg:{resp.Message}"); }
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getOPDReturnStorageByHand({qry.PatNo}...) 失敗:{ex}"); return null; }
            return null;
        }
        ...
        public async Task<List<Data_IPDPatSeq>> getIPDPatSeq(Qry_IPDPatSeq qry)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (resp.Succ) { return resp.Data; }
                    else { qwFunc.log($"Api getIPDPatSeq({qry.PatNo}...) 回傳 Msg:{resp.Message}"); }
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getIPDPatSeq({qry.PatNo}...) 失敗:{ex}"); return null; }
            return null;
        }
        ...
        public async Task<List<Data_IPDReturn>> getIPDReturnStorage(Qry_IPDReturn qry)
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (resp.Succ) { return resp.Data; }
                    else { qwFunc.log($"Api getIPDReturnStorage({qry.PatNo}...) 回傳 Msg:{resp.Message}"); }
                }
            }
            catch (Exception ex) { qwFunc.log($"Api getIPDReturnStorage({qry.PatNo}...) 失敗:{ex}"); return null; }
            return null;
        }
